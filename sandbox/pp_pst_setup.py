...
# ---- PERMH handled with pilot points

par = 'permh'
zone = 1
izone = np.ones( (nlay, nrow, ncol) )
mm.add_param(par, default_value = 1e-4)
...
for lay in range(nlay):
    ...
    kfac_df = ok.calc_factors(x_coords, y_coords, minpts_interp=1, maxpts_interp=50,
                         search_radius=800,pt_zone=1 )
    # write kriging factors to file
    kfac_file1 = os.path.join(mm.mldir,'param','kfac_{0}_l{1:02d}.dat'.format(par,lay+1))
    ok.to_grid_factors_file(kfac_file1)
...
#par_files = [os.path.join(par_dir, f) for f in sorted(os.listdir( par_dir )) if f.endswith('.dat') and f.startwith('pp')]
...
for stat in ['parlbnd','parval1','parubnd']:
    for lay in range(nlay):
        pst.parameter_data.loc[ mm.param[par].pp_dic[lay].index , "pargp"] = par
